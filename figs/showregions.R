...
# County, state, and ecoregion

# Dissolve county map to states
state_map <- county_map %>%
  group_by(STATEFP) %>%
  summarize()

# Clip the tnc map to get rid of the ugly excess bits.
tnc_map_clip <- st_intersection(tnc_map, st_union(county_map))

# Get fill scale for all the realms
realm_names <- unique(tnc_map_clip$WWF_MHTNAM)
...
p48 <- ggplot() +
  geom_sf(aes(fill = WWF_MHTNAM), data = tnc_map_clip %>% filter(!tnc_ak_idx, !tnc_hi_idx), size = 0.5, color = 'gray70') +
  geom_sf(data = county_map %>% filter(!county_ak_idx, !county_hi_idx), size = 0.25, color = 'black', fill = 'transparent') +
  geom_sf(data = state_map %>% filter(!STATEFP %in% c('02', '15')), size = 0.75, color = 'black', fill = 'transparent') +
  realm_fill_scale +
...
pak <- ggplot() +
  geom_sf(aes(fill = WWF_MHTNAM), data = tnc_map_clip %>% filter(tnc_ak_idx) %>% st_transform(ak_crs), size = 0.5, color = 'gray70') +
  geom_sf(data = county_map %>% filter(county_ak_idx) %>% st_transform(ak_crs), size = 0.25, color = 'black', fill = 'transparent') +
  geom_sf(data = state_map %>% filter(STATEFP %in% c('02')) %>% st_transform(ak_crs), size = 0.75, color = 'black', fill = 'transparent') +
  realm_fill_scale + 
  theme(legend.position = 'none')
...
# Bounding box set by TNC clipped map
hi_box <- tnc_map_clip %>% filter(tnc_hi_idx) %>% st_transform(hi_crs) %>% st_bbox

phi <- ggplot() +
  geom_sf(aes(fill = WWF_MHTNAM), data = tnc_map_clip %>% filter(tnc_hi_idx) %>% st_transform(hi_crs), size = 0.5, color = 'gray70') +
  geom_sf(data = county_map %>% filter(county_hi_idx) %>% st_transform(hi_crs), size = 0.25, color = 'black', fill = 'transparent') +
  geom_sf(data = state_map %>% filter(STATEFP %in% c('15')) %>% st_transform(hi_crs), size = 0.75, color = 'black', fill = 'transparent') +
...
ggsave('data/cfs_io_analysis/scenario_v2_figs/usa_ecoregions.png', height = 7, width = 10, dpi = 300)

# Global ecoregion map ----------------------------------------------------

# Get fill scale for all the realms
realm_names_extra <- setdiff(global_eco_map$WWF_MHTNAM, tnc_map_clip$WWF_MHTNAM)
