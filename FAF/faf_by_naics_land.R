...
county_land <- read_csv(file.path(fp_out, 'cropland_by_county_FAF_with_annual.csv'))
...
state_x_faf_land <- county_land %>%
  ...
  summarize(cropland = sum(cropland, na.rm = TRUE), 
            annual_cropland = sum(annual_cropland, na.rm = TRUE),
            permanent_cropland = sum(permanent_cropland, na.rm = TRUE),
            pastureland = sum(pastureland, na.rm = TRUE))
...
state_x_faf_land <- state_x_faf_land %>%
  ungroup %>%
  add_row(state_code = '11', state_abbrev = 'DC', FAF_Region = "Washington-Arlington-Alexandria, DC-VA-MD-WV CFS Area (DC Part)", cropland = 0, annual_cropland = 0, permanent_cropland = 0, pastureland = 0)

state_x_faf_proportions <- state_x_faf_land %>%
  group_by(state_code, state_abbrev) %>%
  mutate(cropland_proportion = cropland/sum(cropland), 
         annual_cropland_proportion = annual_cropland/sum(annual_cropland),
         permanent_cropland_proportion = permanent_cropland/sum(permanent_cropland),
         pastureland_proportion = pastureland/sum(pastureland),
         annual_fraction = annual_cropland/cropland) %>%
  select(-cropland, -pastureland, -annual_cropland, -permanent_cropland)
...
nass_land_normalized <- nass_data %>%
  ...
  mutate(cropland_normalized = cropland * cropland_proportion, 
         annual_cropland_normalized = cropland * annual_cropland_proportion * annual_fraction,
         permanent_cropland_normalized = cropland * permanent_cropland_proportion * (1 - annual_fraction),
         pastureland_normalized = pastureland * pastureland_proportion)
...
# # This section commented out because the products aren't used and are probably incorrect.
